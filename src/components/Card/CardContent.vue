...
<template>
  <div class="pb-2 flex rounded-b-sm border-b-2" :class="[type === 'resource' ? 'border-blue-500' : 'border-red-500']">
    <h3
      class="w-8/12 text-start text-xl font-bold text-nowrap text-ellipsis overflow-x-hidden ease-in-transition"
      :class="[type === 'resource' ? 'hover:text-blue-300' : 'hover:text-red-300']"
    >
      {{ cardInfo.title }}
    </h3>
